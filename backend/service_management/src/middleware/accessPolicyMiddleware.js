  ...
  isAccessAuthorized(req, res, next) {
    try {
      ...
      if (username == "admin") {
        ...
        var files = glob.sync(`${config.FILE_UPLOAD_DIRECTORY}/*-${req.params.fileId}-*`);
        ...
        if (files.length > 1 || !fs.existsSync(fileUploadPath)) {
          ...
          var notFoundErr = new Error("You don't have access to this ressource.");
        ...
        if(filename.split("-")[0] == username) {
          ...
          var noAccessErr = new Error("You don't have access to this ressource.");
